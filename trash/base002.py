...
Flask、Django 等框架是你寫的程式，
Nginx、Gunicorn、uWSGI 等 Web 伺服器是負責接收外部 HTTP 請求的工具，
WSGI 就是讓兩者可以合作溝通的橋樑！

WSGI 解決了什麼問題？
在 WSGI 出現之前，不同 Web 框架（例如 Flask、Django）都用自己的方式處理請求，不相容。

WSGI 統一了介面，讓：
Web框架 >>> 不需要關心伺服器細節
伺服器 >>> 不需要知道應用邏輯
開發者 >>> 可以自由選擇任意伺服器搭配任意框架，只要都支援 WSGI
